...
void ST7305_U8g2::_cmd(uint8_t cmd) {
  ...
  _spi->transfer(cmd); digitalWrite(_cs, HIGH);
}

void ST7305_U8g2::_cmd_data(uint8_t cmd, const uint8_t *data, size_t len) {
  digitalWrite(_dc, LOW); digitalWrite(_cs, LOW); _spi->transfer(cmd);
  if (len) { digitalWrite(_dc, HIGH); _spi->transferBytes((uint8_t *)data, nullptr, len); }
  digitalWrite(_cs, HIGH);
}

void ST7305_U8g2::reset() {
  digitalWrite(_rst, HIGH); delay(50); digitalWrite(_rst, LOW); delay(20);
  digitalWrite(_rst, HIGH); delay(50);
}

uint8_t ST7305_U8g2::u8x8_byte_custom(u8x8_t *, uint8_t, uint8_t, void *) { return 1; }

uint8_t ST7305_U8g2::u8x8_d_st7305_custom(u8x8_t *u8x8, uint8_t msg, uint8_t, void *arg_ptr) {
  if (!instance) return 0;
  switch (msg) {
    case U8X8_MSG_DISPLAY_SETUP_MEMORY:
      u8x8_d_helper_display_setup_memory(u8x8, &display_info); break;
    case U8X8_MSG_DISPLAY_INIT:
      instance->fullInit(); break;
    case U8X8_MSG_DISPLAY_DRAW_TILE: {
      ...
      instance->_cmd_data(0x2C, output, send_count * 4U); break;
    }
    default: return 0;
  }
  return 1;
...
void ST7305_U8g2::fullInit() {
  ...
  _cmd(0x11); delay(120); _cmd_data(0xC9,c9,sizeof(c9));
  _cmd_data(0x36,m36,sizeof(m36)); _cmd_data(0x3A,m3a,sizeof(m3a));
  _cmd_data(0xB9,b9,sizeof(b9)); _cmd_data(0xB8,b8,sizeof(b8)); _cmd(0x21);
